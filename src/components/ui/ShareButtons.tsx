...
import { buildShareUrl, shareNetworks, type ShareNetwork } from '../../utils/shareLinks'
...
function ShareIcon({ network }: { network: ShareNetwork | 'link' | 'native' }) {
  switch (network) {
    case 'x':
      return (
...
export function ShareButtons({
  ...
  const links = useMemo(
    () =>
      shareNetworks.map((network) => ({
        ...network,
        href: buildShareUrl(network.id, shareUrl, shareTitle),
      })),
    [shareUrl, shareTitle],
  )
  ...
  return (
    <div
      className={`share-buttons${compact ? ' share-buttons--compact' : ''} ${className}`.trim()}
      role="group"
      aria-label={label}
    >
      {!compact && <span className="share-buttons__label">{label}</span>}
      <ul className="share-buttons__list">
        {links.map((item) => (
          <li key={item.id}>
            <a
              href={item.href}
              className={`share-buttons__btn share-buttons__btn--${item.id}`}
              target="_blank"
              rel="noopener noreferrer"
              aria-label={item.label}
            >
              <ShareIcon network={item.id} />
              {!compact && <span>{item.label.replace('Share on ', '')}</span>}
            </a>
          </li>
        ))}
        <li>
        ...
        {canNativeShare && (
          <li>
            <button
              type="button"
              className="share-buttons__btn share-buttons__btn--native"
              onClick={nativeShare}
              aria-label="Share via device"
            >
              <ShareIcon network="native" />
